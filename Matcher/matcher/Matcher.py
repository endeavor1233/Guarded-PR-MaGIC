...
class Matcher:
    ...
    def mask_generation(self, tar_img_np, points, box, all_ponits, ref_masks_pool, C):
        ...
        tar_masks = torch.cat(
            ...
            qmask in tar_masks_ori], dim=0).detach().cpu().numpy() > 0
        pred_logits = torch.cat(
            [qmask['logits'][None, None, ...].to(self.device) for
            qmask in tar_masks_ori], dim=0)
        ...
        def check_pred_mask(pred_masks):
            ...
            return pred_masks
        def check_pred_logits(pred_logits):
            if len(pred_logits.shape) < 3:  # avoid only one mask
        ...
        if self.score_filter_cfg["score_filter"]:
            ...
            logits = pred_logits[rank[filer_dis][:self.num_merging_mask]]
            ...
            topk_samples = samples[topk_idx].detach().cpu().numpy()
            topk_scores = scores[topk_idx].detach().cpu().numpy()
            topk_pred_masks = pred_masks[topk_idx]
            topk_pred_logits = pred_logits[topk_idx]
            ...
            topk_pred_logits = check_pred_logits(topk_pred_logits)
            topk_pred_logits = topk_pred_logits[idx]
            mask_list = []
            logit_list = [] 
            for i in range(len(topk_samples)):
                mask = topk_pred_masks[i][None, ...]
                logit =topk_pred_logits[i][None, ...]
        ...
        gradient_feature = torch.autograd.grad(outputs=logits, inputs=self.generator.predictor.features,
                        grad_outputs=torch.ones_like(logits),
                        create_graph=True, retain_graph=False)[0]
        f = 'KL'
        if f == 'KL':
            # s = F.interpolate(logit.unsqueeze(0).unsqueeze(0), size=(64,64), mode="bilinear")
            # s = F.sigmoid(s.detach())>0.5
            s = torch.ones_like(self.generator.predictor.features.detach())
        elif f == 'logD':
            s = 1 / (1 + self.generator.predictor.features.detach().exp())
        elif f == 'JS':
            s = 1 / (1 + 1 / self.generator.predictor.features.detach().exp())
        # clamp 적용.
        eta = self.eta
        gamma = self.gamma
        noise_factor = np.sqrt(gamma) 
        v = s.data * torch.clamp(gradient_feature.data, min=-0.1, max=0.1).detach()
        self.generator.predictor.grad_map = self.generator.predictor.grad_map +\
                eta * v + np.sqrt(2*eta*gamma) * torch.randn_like(self.generator.predictor.features)
        self.generator.predictor.reset_image()
        end_time = time.time()

        elapsed_time = end_time-start_time 
        print(f"gradient flow calculating time : {elapsed_time}")
        del pred_logits

        return torch.tensor(masks, device=self.device, dtype=torch.float).detach()
    ...
    def clear(self):
        ...
        del self.ref_imgs 
        del self.ref_masks_pool 
        del self.nshot 

        del self.encoder_img_size 
        del self.encoder_feat_size 
