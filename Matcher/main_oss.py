...
def test(matcher, dataloader, args=None):
    ...
    for idx, batch in enumerate(dataloader):
        ...
        for t in range(args.nested):
            ...
            average_meter_list[t].write_process(idx, len(dataloader), epoch=-1, write_batch_idx=1, nested=t+1)
    ...
    x=[average_meter_list[i].compute_iou() for i in range(args.nested) ]
    return x
...
if __name__ == '__main__':
    ...
    args.device = device
    Logger.info(f'# using {args.device} available GPUs: %d' % torch.cuda.device_count())
    ...
    # Test Matcher   
    result_list = test(matcher, dataloader_test, args=args)

    for idx ,test_miou, test_fb_iou, _  in enumerate(result_list):   
        Logger.info('[nested %d] Fold %d mIoU: %5.2f \t FB-IoU: %5.2f' % (idx, args.fold, test_miou.item(), test_fb_iou.item()))
