...
def _safe_pascal_miou(candidate_mask, batch):
    """Evaluate Pascal masks without mutating query_ignore_idx.

    The shared Evaluator historically modifies the ignore tensor in-place.
    That is harmless for COCO/FSS, but Pascal carries query_ignore_idx and a
    failed boundary assertion can be swallowed by the episode loop, producing
    a misleading all-zero mIoU export.  This path mirrors the two-class mIoU
    calculation while excluding Pascal ignore pixels.
    """
    pred = torch.as_tensor(candidate_mask, dtype=torch.bool)
    gt = batch['query_mask'].detach().cpu().bool()
    ignore = batch.get('query_ignore_idx')
    if ignore is None:
        raise ValueError('Pascal safe evaluation requires query_ignore_idx')
    ignore = ignore.detach().cpu().bool()
    if pred.ndim == 2:
    ...
    if pred.shape != gt.shape or gt.shape != ignore.shape:
        raise ValueError(
            f'Pascal evaluation shape mismatch: pred={tuple(pred.shape)} '
            f'gt={tuple(gt.shape)} ignore={tuple(ignore.shape)}'
        )
    valid = ~ignore
    pred_fg = pred & valid
    gt_fg = gt & valid
    inter_fg = (pred_fg & gt_fg).sum().float()
    union_fg = (pred_fg | gt_fg).sum().float()
    pred_bg = (~pred) & valid
    gt_bg = (~gt) & valid
    ...
    return float(
        0.5 * (
            inter_fg / (union_fg + 1e-10)
            + inter_bg / (union_bg + 1e-10)
        )
    )
...
def main():
    ...
    for batch_idx, batch in enumerate(p_bar):
        ...
        if args.benchmark == 'pascal' and processed_samples <= 3:
            query_fg = float(batch['query_mask'].detach().float().sum().item())
            query_ignore = batch.get('query_ignore_idx')
            ignore_count = (
                int(query_ignore.detach().bool().sum().item())
                if query_ignore is not None else 0
            )
            print(
                f'[pascal visual check] {obj_name}: '
                f'query_shape={tuple(batch["query_mask"].shape)} '
                f'query_fg_pixels={query_fg:.0f} '
                f'ignore_pixels={ignore_count}'
            )
    ...
    for t in range(T):
        ...
        try:
            if args.benchmark == 'pascal':
                miou = _safe_pascal_miou(candidate_mask, batch)
            else:
            ...
            warnings.warn(f' mIoU evaluation failed at candidate {t}: {eval_error}')
            miou = 0.0
