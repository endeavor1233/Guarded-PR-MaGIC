...
class SamPredictor:
    ...
    def encode_image(
      ...
      image_format: str = "RGB",
        ...
        return self.model.image_encoder(input_image_torch)
    ...
    def set_image_grad(
        ...
        input_mask = self.set_torch_image_grad(input_image_torch, image.shape[:2], transformed_mask=input_mask_torch)
        return input_mask
    
    def set_torch_image_grad(
        self,
    ...
    def predict_sim(
        ...
        if mask_input is not None:
            mask_input_torch = torch.as_tensor(mask_input, dtype=torch.float, device=self.device)
            mask_input_torch = mask_input_torch[None, :, :, :]
        masks, iou_predictions, low_res_masks, high_res_masks = self.predict_torch_sim(
            coords_torch,
            labels_torch,
            box_torch,
            mask_input_torch,
    ...
    def predict_torch_sim(
        ...
        with torch.no_grad():
          input_sim = self.transform.apply_image(sim_image)
          input_sim_torch = torch.as_tensor(input_sim, device=self.device)
          input_sim_torch = input_sim_torch.permute(2, 0, 1).contiguous()[None, :, :, :]
          input_sim_final = self.model.preprocess(input_sim_torch)
        # Predict masks
        low_res_masks, iou_predictions = self.model.mask_decoder(
            image_embeddings=self.model.image_encoder(input_sim_final),
