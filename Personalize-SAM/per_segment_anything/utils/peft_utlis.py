# function to apply peft to the image_encoder (sam.image_encoder)
...
def apply_lora2image_encoder_attention(model, device, rank=10, lora_alpha=1, lora_q= True, lora_k= True, lora_v= False): 

    for i in range(len(model.image_encoder.layers)):
        layer = model.image_encoder.layers[i].attn
        ve_attention_layer_parameterization(layer, layer.qkv.weight.device, rank, lora_alpha=lora_alpha)


def enable_disable_lora_sam_image_encoder(model, enabled= True): 
