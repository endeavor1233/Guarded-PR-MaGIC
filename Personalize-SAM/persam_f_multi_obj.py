...
def get_arguments():
    ...
    parser.add_argument('--nested', type=int, default=25)
    parser.add_argument('--eta', type=float, default=1e-3)
    parser.add_argument('--gamma', type=float , default = 1e-10)
    parser.add_argument('--f', type=str, default='KL')
    parser.add_argument('--gt_path', type=str, default='./data/Perseg/Annotations')
    ...
    args.outdir = 'multi_'+args.data.split('/')[2] + '_' + str(args.eta) + '_' + str(args.gamma) + '_' + str(args.nested) + '_'  + args.f + '_bi'
...
def main():
    ...
    if args.all:
        ...
        class_names = ["ab_wheel","abacus", "ac_ground" ,"abe's_flyingfish", "ac_wall", "accordion","acorn","adhensive_tape", 'adidas_logo1', 'adidas_logo2', 'afghan_hound']
    class_names.sort()

    if args.end > len(class_names):
        args.end = len(class_names)

    print("\n------------>Experiment<------------")
    for obj_name in tqdm(class_names[args.start:args.end]):
...
def persam_f(args, obj_name, images_path, masks_path, output_path):
    ...
    for test_idx in tqdm(range(len(os.listdir(test_images_path)))):
        ...
        for i in tqdm(range(args.max_objects)):
            ...
            logits = logits.cpu() * weights_np[..., None]
            logit = logits.sum(0)
